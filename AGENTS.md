...
- **System access is in scope. Dispach is a harness, not a channel-resident assistant** — peer to
  OpenClaw, Hermes Agent and Codex. It runs shell commands and touches files because that is
...
- **A spec limit that upstream violates is a warning, never a load failure.** `anthropics/skills`'
  `Codex-api` carries a **1,068**-character description against the spec's 1,024, and `awesome-copilot`
