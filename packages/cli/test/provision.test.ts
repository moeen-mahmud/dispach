...
import { dirFor, SECRET_STEPS, STEP_ORDER } from "#lib/init-flow"
import { FLAG_FOR, provisionAgent, provisionSteps, toProvisionStep } from "#lib/provision"
...
describe("every answer the wizard has is reachable at the terminal", () => {
    /**
     * **A step declared in `InitAnswers` and absent from `STEP_ORDER` is a question nobody is ever
     * asked** — and it is silent in both directions that matter: the flag still works, so a
     * scripted run and every test using one passes, while the interactive wizard skips it and
     * `GET /v1/provision` omits it from the browser's form.
     *
     * That is exactly what happened to `whatsapp`, and it survived a manifest that generated
     * correctly, a validate that said ok, and a container run — because all of them went through
     * `--whatsapp`. What caught it was asking the *served* list, one layer further out. This test
     * is that question, asked structurally: `boundaries.test.ts` already walks every `CommandSpec`
     * flag into `index.ts`, and nothing walked the wizard's own steps.
     *
     * Four steps are **deliberately** flag-only, and each is here by name with the reason rather
     * than by a rule that would let a fifth in silently.
     */
    const FLAG_ONLY: ReadonlyMap<string, string> = new Map([
        // Withdrawn after use (9.17): nobody knows they want a schedule at minute two.
        ["schedules", "withdrawn as a question, kept as a flag"],
        // Withdrawn in 0.1.2: asking whether you want the product is not a question.
        ["server", "the server is on by default; --server none is the opt-out"],
        // A picker cannot run in CI, so the interactive and scripted paths differ by design.
        ["skillsSearch", "the catalogue picker's scripted twin"],
        ["skillsPick", "the catalogue picker's scripted twin"],
        // Derived from the chosen preset — asking twice for something a preset already decided.
        // Its comment in `complete()` is the origin of this whole class of defect.
        ["apiKeyEnv", "derived from the preset, overridable with --api-key-env"],
        // Minted, never answered: a token somebody typed is a token in shell history.
        ["serverToken", "generated by complete(), which is why FLAG_FOR says (generated)"],
    ])

    test("every InitAnswers key is either asked or knowingly flag-only", () => {
        const asked = new Set<string>(STEP_ORDER)
        const orphans = Object.keys(FLAG_FOR).filter(
            (step) => !asked.has(step) && !FLAG_ONLY.has(step),
        )
        expect(orphans).toEqual([])
    })

    test("and the flag-only list names nothing that is actually asked", () => {
        // The other direction, which is what stops this test rotting into a rubber stamp: a step
        // listed as flag-only *and* asked means the reason written beside it is no longer true.
        const asked = new Set<string>(STEP_ORDER)
        expect([...FLAG_ONLY.keys()].filter((step) => asked.has(step))).toEqual([])
    })

    test("the browser is offered every step the terminal asks, minus the refused ones", () => {
        // `provisionSteps` is walked from `nextQuestion`, so this cannot drift by itself — what it
        // catches is the case above, where a step is missing from the order both of them read.
        const served = new Set(
            provisionSteps({ agentDirBase: "/tmp/provision-order" }).map((s) => s.step),
        )
        const missing = STEP_ORDER.filter(
            (step) => !served.has(step) && !REFUSED_FROM_THE_WIRE.has(step),
        )
...
/** What `REFUSED_STEPS` holds: a provisioned agent's directory, and installing a service unit. */
const REFUSED_FROM_THE_WIRE = new Set(["dir", "dirChoice", "daemon"])
